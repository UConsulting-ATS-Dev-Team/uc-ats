 ...
 *
 * Deliberately not where rosters live. This page and Interview Scheduling used
 * to both show who was in which session, which left a permanent "which page do
 * I use" question and two ways to change the same thing. The split now follows
 * the job: Scheduling answers *who is where and when*, this answers *make an
 * interview* and *run one today*.
 */
...
/// Where a live session of each type is run. Unchanged from before - these
/// routes and their ?groupIds= contract are what the interview interfaces read.
...
export default function AdminAssignedInterviews() {
  ...
  /** Launch the live interface for the sessions this interviewer is running. */
  ...
  const openQuestions = async (interview) => {
    const roster = rostersById[interview.id];
    const first = roster?.slots?.[0]?.id ?? '';
    setQuestionsFor(interview);
    setQuestionSession(first);
    setQuestions([]);
    if (!first) return;
    try {
      const config = await apiClient.get(`/admin/interviews/${interview.id}/config?groupIds=${first}`);
      const byGroup = config.behavioralQuestions || {};
      setQuestions(Object.values(byGroup).flat().map((q) => q.questionText ?? q));
    } catch {
      setQuestions([]);
    }
  };
  ...
  return (
    <AccessControl allowedRoles={['ADMIN']}>
      <Container maxWidth="lg" sx={{ py: 3 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start" sx={{ mb: 1 }}>
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              {cycle?.name ?? 'No active cycle'} — create an interview, then run its sessions.
            </Typography>
          </Box>
        ...
        {/* Says where the other half of the job lives, so nobody has to guess
            which of two pages owns rosters. */}
        <Alert severity="info" sx={{ mb: 3 }}>
          Times, seats, rosters and waitlists live in{' '}
          <Link component={RouterLink} to="/admin/interview-scheduling" fontWeight={600}>
            Interview Scheduling
          </Link>
          . This page is for creating interviews and running them on the day.
        </Alert>
        ...
        {interviews.length === 0 && (
          <Card variant="outlined">
            <CardContent sx={{ textAlign: 'center', py: 5 }}>
              ...
              <Typography variant="body2" color="text.secondary">
                Create one, then set up the sessions candidates book into.
              </Typography>
            </CardContent>
          </Card>
        )}
          ...
          <Box key={label} sx={{ mb: 4 }}>
            ...
            <Stack spacing={2} sx={{ mt: 1 }}>
              {group.map((interview) => {
                ...
                const unscheduled = roster?.unassigned?.length ?? 0;
                  ...
                  <Card key={interview.id} variant="outlined">
                    <CardContent>
                      ...
                      <Stack direction="row" spacing={1} sx={{ mt: 2, flexWrap: 'wrap', gap: 1 }}>
                        ...
                        {unscheduled > 0 && (
                          <Chip size="small" color="warning" label={`${unscheduled} not scheduled`} />
                        )}
                        {sessions.length === 0 && (
                          <Chip size="small" color="warning" label="No sessions yet" />
                        )}
                      </Stack>
                    </CardContent>
        ...
        <Dialog open={createOpen} onClose={() => setCreateOpen(false)} fullWidth maxWidth="sm">
          ...
          <DialogContent>
            <Stack spacing={2} sx={{ mt: 1 }}>
              ...
              />
              <Alert severity="info">
                After creating it, add the sessions candidates book into from Interview Scheduling.
              </Alert>
            </Stack>
          </DialogContent>
          <DialogActions>
          ...
          <DialogContent>
            <TextField
              ...
              onChange={async (e) => {
                ...
                setQuestionSession(id);
                try {
                  const config = await apiClient.get(
                    `/admin/interviews/${questionsFor.id}/config?groupIds=${id}`
                  );
                  setQuestions(Object.values(config.behavioralQuestions || {}).flat().map((q) => q.questionText ?? q));
                } catch {
                  setQuestions([]);
                }
              }}
            ...
            <Stack spacing={1}>
              {questions.map((q, index) => (
                <TextField
                  ...
                  value={q}
                  onChange={(e) =>
                    setQuestions((current) => current.map((item, i) => (i === index ? e.target.value : item)))
                  }
                  fullWidth
                />
              ))}
