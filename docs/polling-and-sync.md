...
## Server version semantics (known gap)

`getVersion` is the intended defence against applying an older server snapshot over a newer
one. Staging cannot use it yet: `Application` has no `updatedAt` column and none of the
staging endpoints return a snapshot version, so Staging relies on the arrival-order guard
only. Closing this properly means adding `updatedAt` to `Application` (a Prisma migration)
and returning it, or exposing a snapshot version on the staging endpoints — deliberately out
of scope here since it touches migrations.
